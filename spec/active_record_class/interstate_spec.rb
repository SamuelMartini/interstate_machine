...
#   let(:record) { ActiveRecord::TrafficLight.new(name: 'Traffic', state: 'caution') }
#
#   describe '#state' do
#     context 'when state is present on record field' do
#       let(:record) { ActiveRecord::TrafficLight.new(name: 'Traffic', state: 'proceed') }
#
...
#       let(:record) { ActiveRecord::TrafficLight.new(name: 'Traffic', state: nil) }
#
